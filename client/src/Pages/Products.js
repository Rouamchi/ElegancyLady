...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const { data } = await axios.get('https://http://localhost:4000/products')
      setProducts(data)
    }
    fetchProducts()
  }, [])
            ...
            <div className="overflow-hidden -mt-10 grid grid-cols-2 gap-x-4 gap-y-8 md:gap-x-6 md:gap-y-10 sm:grid-cols-2 md:grid-cols-4 xl:gap-x-8">
              {products.map((singleProduct) => (
                <div to={singleProduct._id} className="group relative">
                  ...
                  <button className="p-2 rounded-full bg-gray-100 text-white mx-28 md:mx-56 -my-24 -mb-4 hover:bg-white focus:outline-none focus:bg-white">
